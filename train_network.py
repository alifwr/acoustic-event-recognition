...
import os, testDataSegmentation, random, numpy as np
...
def cnnModel():
    ...
    model = Sequential()
    model.add(Conv2D(64, (1,1), input_shape=inputShape, activation='relu', padding='valid'))
    model.add(BatchNormalization())
    model.add(Conv2D(64, (3,3), activation='relu', padding='valid'))
    model.add(BatchNormalization())
    model.add(MaxPooling2D(pool_size=(3,2)))
    model.add(Conv2D(32, (3,1), activation='relu', padding='valid'))
...
if __name__ == '__main__':
    ...
    model.summary()
    # Training the model
    history = model.fit(X_train,Y_train, validation_data=(X_validation,Y_validation),epochs=50,batch_size=100,verbose=2)    # Testing the model
