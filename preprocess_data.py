...
def main(inpath='Samples/', outpath='Preproc/'):
    ...
    for idx, classname in enumerate(class_names):   # go through the subdirs
        ...
        for idx2, infilename in enumerate(class_files):
            ...
            aud, sr = librosa.load(audio_path, sr=8000) # reading the content of the files
            print(audio_path)
            melgram = librosa.amplitude_to_db(librosa.feature.melspectrogram(aud, sr=sr, n_mels=nMels))
            melgram = melgram[np.newaxis,:,2:78,np.newaxis]
            #print(melgram.shape)
